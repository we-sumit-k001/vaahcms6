...
class VaahThemeRegistration extends Registration
{
    ...
    public static function postCreate($request)
    {
        ...
        // in Here i add the code if user click on the Signup intially the status will be email-verification-pending
        if ($item->status === null) {
        ...
        $response['data']['redirect_url'] = route('vh.frontend.vaahtheme.signin');
	...
	public static function activateRegistration($request)
    {
        ...
        // Create User
        $user = VaahThemeUser::createUser($user_request);

        if($user)
        {
            $signup->vh_user_id = $user->id;
            // Here i make this status  verified when user verified his email
